...
test_that('deactivate_variables', {
  ...
  expect_error(deactivate_input_variables(lines = hectordata::template_ini, vars = c(x, 'fake')))
...
test_that('activate_variables', {
  ...
  # Make sure that it throws an error 
  expect_error(activate_input_variables(lines = hectordata::template_ini, vars = c(x, 'fake')))
...
test_that('replace_csv_string', {
  ...
  out_ini <- replace_csv_string(ini = hectordata::template_ini,  
