...
expect_error(ud_convert2(x = '1', from = 'fake', 'g'))
})
...
test_that('complete_missing_years', {
  ...
  expect_error(complete_missing_years(data = test_data[, list(variable)], expected_years = 1990:2000))
  
  out <- complete_missing_years(data = test_data, expected_years = 1990:2000)
  expect_true(all(1990:2000 %in% out$year))
...
test_that('process_carbon_cycle_emissions', {
  ...
  df <- data.frame("year" = 1900, "variable"= EMISSIONS_BC(),"units" = "fake", "value" = 10)
  ...
  df <- data.frame("year" = 1900, "variable"= FFI_EMISSIONS(),"scenario" = "fake", "value" = 10, 
  ...
  df <- data.table(year = rep(1905:1910, 2), 
                   variable= rep(x = c(FFI_EMISSIONS(), LUC_EMISSIONS()), each = 6), 
                   scenario = "ssp119", 
                   units = getunits(FFI_EMISSIONS()), 
                   value = rep(2*5:10, 2)) 
