...
convert_rcmipCMIP6_hector <- function(scenario = NULL){
  ...
  conc_data <- data.table::as.data.table(readr::read_csv(url("https://zenodo.org/record/3779281/files/rcmip-concentrations-annual-means-v4-0-0.csv?download=1")))
  emiss_data <- data.table::as.data.table(readr::read_csv(url("https://zenodo.org/record/3779281/files/rcmip-emissions-annual-means-v4-0-0.csv?download=1")))


  # Make sure that the scenarios can be processed.
  ...
  raw_inputs <- na.omit(rbind(emiss_long, conc_long))
  raw_inputs <- raw_inputs[Scenario %in% scenario  & Region == "World"][ , year :=  as.integer(year)]
  raw_inputs <- remove_spaces(raw_inputs, cols = c("Variable", "Mip_Era", "Model", "Scenario", "Region"))
  ...
  input_conversion_table <- na.omit(raw_inputs[conversion_table, on = c('Variable' = 'rcmip_variable'), nomatch=NA])
