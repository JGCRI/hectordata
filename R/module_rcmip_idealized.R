...
module_rcmip_idealized <- function(){
  ...
  suppressWarnings({vals <- cdr_co2_vals[Variable == "Atmospheric Concentrations|CO2", ..yr_cols]})
  suppressWarnings({cdr_df[(cdr_df$Variable == "Atmospheric Concentrations|CO2" & cdr_df$Region == "World"),  ..yr_cols]#, names(cdr_df) %in% yr_cols] <- vals
}) 
  ...
  suppressWarnings({vars_to_add <- vars_to_add[data.table::as.data.table(hectordata::rcmipCMIP6_conversion)[ , ..cols], nomatch=0, on = "rcmip_variable" ]})
  vars_to_add <- unique(vars_to_add)
  vars_to_add$join <- "on"
  
  cols <- c("Model", "Scenario", "Region", "Activity_Id", "Mip_Era")
  suppressWarnings({scn_info <- raw_inputs[, ..cols ]})
...
#' 
#' @param f string path to the csv table files 
#' @return str of the ini files
#' @noRd
